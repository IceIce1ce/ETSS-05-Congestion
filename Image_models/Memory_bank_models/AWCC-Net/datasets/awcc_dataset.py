...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size, downsample_ratio, method='train'):
        ...
        self.trans = transforms.Compose([transforms.ToTensor(), transforms.Normalize(*MEAN_STD)])
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(img_path).convert('RGB') # [512, 814, 3]
        if self.method == 'train':
            keypoints = np.load(gd_path)
            images, points, targets, st_sizes = self.train_transform(img, keypoints)
            return images, points, targets, st_sizes
        if self.method == 'val':
            keypoints = np.load(gd_path) # [463, 2]
            img = self.trans(img) # [3, 512, 814]
