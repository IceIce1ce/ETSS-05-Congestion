...
class AWCCNet(nn.Module):
    def __init__(self, use_pe=False, output_dim=1):
        ...
        self.decoder = nn.ModuleList([Block_dec(dim=512, num_heads=8, mlp_ratio=4, qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=dpr[i], norm_layer=partial(nn.LayerNorm, eps=1e-6)) for i in range(depths[0])])
    ...
    def forward(self, x): # [1, 3, 955, 1300]
        b, c, h, w = x.shape
        rh = int(h) // 16
        rw = int(w) // 16
        vgg_feats = self.features(x) # [1, 512, 29, 40]
        self.weight_vectors = self.weather_encoder(vgg_feats, return_logit=True)
        self.weight_vectors = F.softmax(self.weight_vectors, dim=-1) # [1, 8]
        # transformer encoder
        bs, c, h, w = vgg_feats.shape
        vgg_feats = vgg_feats.flatten(2).permute(0, 2, 1) # [1, 1160, 512]
        vgg_feats = vgg_feats[:1]
        if self.pos_encoder is not None:
            vgg_feats = self.pos_encoder(vgg_feats, h, w, nbc=False) # [1, 1160, 512]
        x = self.encoder(vgg_feats) # [1, 1160, 512]
        # transformer decoder
        style_q = torch.einsum('bs, snc -> bnc', self.weight_vectors, self.weather_bank) # [1, 48, 512]
        for blk in self.decoder:
            x = blk(x, h, w, style_q[:1]) # [1, 1160, 512]
        # regression head
        x = x.permute(0, 2, 1).view(1, c, h, w) # [1, 512, 29, 40]
        x = F.interpolate(x, size=(rh, rw), mode='bilinear', align_corners=True) # [1, 512, 29, 81]
        x = self.reg_decoder(x) # [1, 1, 59, 81]
...
class PositionalEncoding2d(nn.Module):
    ...
    def forward(self, x: Tensor, h, w, nbc=True) -> Tensor: # [1, 1160, 512]
        if h != 16 or w != 16:
...
class WeatherEncoder(nn.Module):
    ...
    def forward(self, x, return_logit=False): # [1, 512, 29, 40]
        feats = self.model(x) # [1, 256, 1, 1]
        if return_logit:
            logits = self.fc(feats).view(x.size(0), -1) # [1, 8]
            return logits
        else:
...
class Conv2dBlock(nn.Module):
    ...
    def forward(self, x): # [1, 512, 29, 40]
