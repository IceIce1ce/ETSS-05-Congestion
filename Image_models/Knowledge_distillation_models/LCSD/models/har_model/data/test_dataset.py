# https://github.com/junleen/RainNet/blob/main/data/test_dataset.py
import os.path
...
class TestDataset(Dataset):
    def __init__(self, dataset_name, foreground_paths, mask_paths, background_paths=None, load_size=256):
        self.foreg_paths, self.mask_paths, self.backg_paths = foreground_paths, mask_paths, background_paths
        self._load_images_paths()
        self._load_sizez = load_size
        self.transform_image = transforms.Compose([transforms.Resize([load_size,load_size]), transforms.ToTensor(), transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))])
        self.transform_mask = transforms.Compose([transforms.Resize([load_size,load_size]), transforms.ToTensor()])
    ...
    def _load_images_paths(self, ):
        if isinstance(self.foreg_paths, list):
            ...
            self.foreg_paths = glob(self.foreg_paths + '/*.jpg') + glob(self.foreg_paths + '/*.png') + glob(self.foreg_paths + '/*.bmp')
            self.mask_paths = glob(self.mask_paths + '/*.png')
            if self.backg_paths is not None:
                self.backg_paths = glob(self.mask_paths + '/*.png') + glob(self.backg_paths + '/*.jpg') + glob(self.backg_paths + '/*.bmp')
    ...
    def __getitem__(self, index):
        ...
        return {'comp': comp, 'mask': mask, 'real': real, 'img_path': self.foreg_paths[index].replace(self.dataset_name, self.dataset_name.replace("compos","harmonized"))}
