...
class RealPredict(Dataset):
    def __init__(self, root_path, method, transform=None):
    ...
    def __getitem__(self, item):
        ...
        boxes = np.zeros((1, 5))
        if self.trans:
            img_tensor, _ = self.trans((img, boxes))
        base_name = os.path.basename(img_path)
        name = base_name.split('.')[0]
        return img, img_tensor, name
...
class ListDataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            img_path = os.path.join("data/coco", img_path[1:])
            img = np.array(Image.open(img_path).convert('RGB'), dtype=np.uint8)
        except Exception:
        ...
        try:
            ...
            label_path = os.path.join("data/coco", label_path[1:])
