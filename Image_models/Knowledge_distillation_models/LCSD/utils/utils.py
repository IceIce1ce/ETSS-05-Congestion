...
def generate_gaussian_kernels(out_kernels_path='gaussian_kernels.pkl', round_decimals = 3, sigma_threshold = 4, sigma_min=0, sigma_max=20, num_sigmas=801):
    ...
    for sigma in tqdm(sigma_space):
        ...
        kernel_size = np.ceil(sigma * sigma_threshold).astype(np.int32)
        img_shape  = (kernel_size * 2 + 1, kernel_size * 2 + 1)
        img_center = (img_shape[0] // 2, img_shape[1] // 2)
        arr = np.zeros(img_shape) # [1, 1]
        arr[img_center] = 1
        arr = gaussian_filter(arr, sigma, mode='constant') # [1, 1]
        kernel = arr / arr.sum()
...
def compute_distances(out_dist_path, images, points, n_neighbors = 4, leafsize=1024):
    ...
    for index, (img, point) in tqdm(enumerate(zip(images, points))):
        ...
        non_zero_points = get_gt_dots(point, height, width) # [42, 2]
        distances = []
        if non_zero_points.shape[0] != 0:
            tree = scipy.spatial.KDTree(non_zero_points.copy(), leafsize=leafsize)
            distances, _ = tree.query(non_zero_points, k=n_neighbors) # [42, 4]
...
def compute_distances_online(images, points, n_neighbors=4, leafsize=1024):
    ...
    for index, (img, point) in enumerate(tqdm(zip(images, points))):
        ...
        if non_zero_points.shape[0] != 0:
            tree = scipy.spatial.KDTree(non_zero_points.copy(), leafsize=leafsize)
            distances, _ = tree.query(non_zero_points, k=n_neighbors)
        distances_dict.append(distances)
    return distances_dict

def get_gt_dots(gt, img_height, img_width): # [42, 2]
...
def gaussian_filter_density(non_zero_points, map_h, map_w, distances=None, kernels_dict=None, img_head_diameters=None, min_sigma=2, method=1, const_sigma=15):
    ...
    density_map = np.zeros((map_h, map_w), dtype=np.float32) # [480, 640]
    if gt_count == 0:
        return density_map
    for i in range(gt_count):
        ...
        point_y = int(point_y)
        try:
            sigma = compute_sigma(gt_count, point_y, distances[i], img_head_diameters[i] if img_head_diameters!= None else None , min_sigma=min_sigma, method=method, fixed_sigma=const_sigma) # 2
