...
class OT_Loss(Module):
    ...
    def forward(self, normed_density, unnormed_density, points): # [10, 1, 48, 48], [10, 1, 48, 48], [42, 2] * len(10)
        batch_size = normed_density.size(0)
        assert len(points) == batch_size
        assert self.output_size == normed_density.size(2)
        loss = torch.zeros([1]).cuda()
        ot_obj_values = torch.zeros([1]).cuda()
        wd = 0
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:
                ...
                target_prob = (torch.ones([len(im_points)]) / len(im_points)).cuda()
        ...
        return loss, wd, ot_obj_values # [1], [1], [1]