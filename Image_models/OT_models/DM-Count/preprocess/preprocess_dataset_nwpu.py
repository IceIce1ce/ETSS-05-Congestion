...
def gen_density_map_gaussian(im_height, im_width, points, sigma=4):
    ...
    for p in points:
        ...
        gaussian_map = np.multiply(cv2.getGaussianKernel(int(gaussian_radius * 2 + 1), sigma), cv2.getGaussianKernel(int(gaussian_radius * 2 + 1), sigma).T)
        ...
        density_map[max(0, p[0] - gaussian_radius):min(h, p[0] + gaussian_radius + 1), max(0, p[1] - gaussian_radius):min(w, p[1] + gaussian_radius + 1)] += gaussian_map
    ...
    for phase in ['train', 'val']:
        ...
        with open(os.path.join(input_dataset_path, '{}.txt'.format(phase))) as f:
            ...
            for i in lines:
                ...
                print('Processing training image:', name)
