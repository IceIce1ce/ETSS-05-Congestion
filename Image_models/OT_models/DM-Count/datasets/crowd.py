...
def gen_discrete_map(im_height, im_width, points):
    ...
    if num_gt == 0:
        return discrete_map
    points_np = np.array(points).round().astype(int) # [7, 2]
    p_h = np.minimum(points_np[:, 1], np.array([h-1]*num_gt).astype(int))
    p_w = np.minimum(points_np[:, 0], np.array([w-1]*num_gt).astype(int))
    p_index = torch.from_numpy(p_h* im_width + p_w)
    discrete_map = torch.zeros(im_width * im_height).scatter_add_(0, index=p_index, src=torch.ones(im_width*im_height)).view(im_height, im_width).numpy() # [384, 384]
...
class Base(data.Dataset):
    ...
    def train_transform(self, img, keypoints):
        ...
        return self.trans(img), torch.from_numpy(keypoints.copy()).float(), torch.from_numpy(gt_discrete.copy()).float() # [3, 384, 384], [28, 2], [1, 48, 48]
