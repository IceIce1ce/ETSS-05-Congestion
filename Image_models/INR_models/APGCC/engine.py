...
class Trainer(object):
    def __init__(self, cfg, model, train_dl, val_dl, criterion):
        ...
        param_dicts = [{"params": [p for n, p in self.model_without_ddp.named_parameters() if "encoder" not in n and p.requires_grad]},
                       {"params": [p for n, p in self.model_without_ddp.named_parameters() if "encoder" in n and p.requires_grad], "lr": self.cfg.SOLVER.LR_BACKBONE}]
    ...
    def handle_new_epoch(self):
        ...
        if self.train_epoch % self.eval_period == 0 and self.train_epoch != 0:
            t1 = time.time()
            # result = evaluate_crowd_no_overlap(self.model, self.val_dl, next(self.model.parameters()).device)
            result = evaluate_crowd_counting(self.model, self.val_dl, next(self.model.parameters()).device, self.cfg.TEST.THRESHOLD, os.path.join(self.cfg.OUTPUT_DIR, 'sample_result_for_val'))
            t2 = time.time()
            self.log_eval.update(result[0], result[1], self.train_epoch)
            self.logger.info('[ep %d] Eval: MAE=%.6f/%.6f, MSE=%.6f/%.6f, Best[ep %d]: MAE=%.6f, MSE=%.6f, time:%.2fs' % (self.train_epoch, result[0], self.log_eval.MAE_avg,
                             result[1], self.log_eval.MSE_avg, self.log_eval.best_ep, self.log_eval.MAE_min, self.log_eval.MSE_min, t2 - t1))
    ...
    def step(self, batch):
        ...
        samples, targets = batch # [32, 3, 128, 128]
        samples = samples.to(device)
        targets = [{k: v.to(device) if k!='name' else v for k, v in t.items()} for t in targets]
        outputs = self.model(samples) # pred_logits = pred_points = offset: [32, 1024, 2]
    ...
    def save(self):
        checkpoint_best_path = os.path.join(self.output_dir, 'best_ep%d__%.5f_%.5f.pth' % (self.log_eval.best_ep, self.log_eval.MAE_min, self.log_eval.MSE_min))
...
@torch.no_grad()
def evaluate_crowd_no_overlap(model, data_loader, device, threshold=0.5):
    model.to(device)
    model.eval()
    maes = []
    mses = []
    counts_pred, counts_true = [], []
    img_id = []
