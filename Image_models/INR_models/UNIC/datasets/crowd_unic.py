...
def get_coordinates(res_original, k_HR: int): # 32, 1
    lower, upper = -0.99, 0.99
    x = np.linspace(lower, upper, res_original) # [32]
    y = np.linspace(lower, upper, res_original) # [32]
    xx, yy = np.meshgrid(x, y) # [32, 32], [32, 32]
    dx_HR, dy_HR = [], []
    for i in range(0, res_original, k_HR):
        ...
        dx_HR.append(tmp_x)
        dy_HR.append(tmp_y)
    d_HR = np.array([dx_HR, dy_HR]) # [2, 32, 32]
    del dx_HR, dy_HR, xx, yy
    return torch.tensor(np.transpose(np.reshape(d_HR, (d_HR.shape[0], d_HR.shape[1] * d_HR.shape[2])), [1, 0]), dtype=torch.float) # [1024, 2]
...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size, downsample_ratio,is_gray=False, method='train'):
        ...
        self.n_query_pts_c = self.cor_sample_size_c * self.cor_sample_size_c
    ...
    def train_transform(self, img, keypoints, prior_prob, den_target):
        ...
        if True:
            mask_prior = np.full(mask.shape[0] +1, True) # [200]
            mask_prior[:-1] = mask
            prior_prob = prior_prob[mask_prior] # [200, 1024]
        den_target = np.reshape(den_target, [1, -1]) # [1, 1024]
        prior_prob = np.concatenate((prior_prob, den_target), axis=0) # [201, 1024]
        if len(keypoints) > 0:
            ...
            if random.random() > 0.5:
                img = F.hflip(img)
        self.gridnum_sam_c = np.array(self.gridnum_list_c) # [1024]
        prior_prob = prior_prob[:, self.gridnum_sam_c] # [213, 1024]
        grid_c = self.cor_C[self.gridnum_sam_c, :] # [1024, 2]
        prior_prob[:-2] = prior_prob[:-2] * target.reshape([-1, 1])
        return self.trans(img), torch.from_numpy(keypoints.copy()).float(), torch.from_numpy(prior_prob.copy()).float(), st_size, grid_c, torch.from_numpy(self.gridnum_sam_c.copy()).float(), gd_count