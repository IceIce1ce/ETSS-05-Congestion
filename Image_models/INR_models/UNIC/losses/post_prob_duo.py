...
class Post_Prob(Module):
    def __init__(self, sigma, c_size, stride, background_ratio, use_background):
        ...
        if stride ==1:
            self.cood = torch.arange(0, c_size, step=stride, dtype=torch.float32).cuda()
        else:
            self.cood = torch.arange(0, c_size, step=stride, dtype=torch.float32).cuda() + stride / 2
        self.cood.unsqueeze_(0)
        self.softmax = torch.nn.Softmax(dim=0)
        self.use_bg = use_background

    def forward(self, points, st_sizes, gridnum_sam_c):
        ...
        if len(all_points) > 0:
            ...
            prob_list = []
            for dis, st_size, gridnum_sample in zip(dis_list, st_sizes, gridnum_sam_c):
                if len(dis) > 0:
                    if self.use_bg:
