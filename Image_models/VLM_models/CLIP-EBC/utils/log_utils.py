...
def update_eval_result(epoch: int, curr_scores: Dict[str, float], hist_scores: Dict[str, List[float]], best_scores: Dict[str, List[float]],
                       writer: SummaryWriter, state_dict: OrderedDict[str, Tensor], ckpt_dir: str) -> Tuple[Dict[str, List[float]], Dict[str, float]]:
...
def log(logger: logging.Logger, epoch: int,  total_epochs: int, loss_info: Optional[Dict[str, float]] = None, curr_scores: Optional[Dict[str, float]] = None,
        best_scores: Optional[Dict[str, float]] = None, message: Optional[str] = None) -> None:
    if epoch is None:
