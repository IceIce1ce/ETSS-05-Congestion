...
def get_loss_fn(args: ArgumentParser) -> nn.Module:
    if args.bins is None:
        ...
        loss_fn = losses.DMLoss(input_size=args.input_size, reduction=args.reduction)
    else:
        loss_fn = losses.DACELoss(bins=args.bins, reduction=args.reduction, weight_count_loss=args.weight_count_loss, count_loss=args.count_loss, input_size=args.input_size)
    return loss_fn
...
def get_optimizer(args: ArgumentParser, model: nn.Module) -> Tuple[Adam, LambdaLR]:
    ...
    scheduler = LambdaLR(optimizer=optimizer, lr_lambda=partial(cosine_annealing_warm_restarts, warmup_epochs=args.warmup_epochs, warmup_lr=args.warmup_lr, T_0=args.T_0,
                         T_mult=args.T_mult, eta_min=args.eta_min, base_lr=args.lr))
    return optimizer, scheduler

def load_checkpoint(args: ArgumentParser, model: nn.Module, optimizer: Adam, scheduler: LambdaLR, grad_scaler: GradScaler) -> Tuple[nn.Module, Adam, Union[LambdaLR, None], GradScaler, int, Union[Dict[str, float], None], Dict[str, List[float]], Dict[str, float]]:
...
def save_checkpoint(epoch: int, model_state_dict: OrderedDict[str, Tensor], optimizer_state_dict: OrderedDict[str, Tensor], scheduler_state_dict: OrderedDict[str, Tensor],
                    grad_scaler_state_dict: OrderedDict[str, Tensor], loss_info: Dict[str, List[float]], hist_scores: Dict[str, List[float]], best_scores: Dict[str, float], ckpt_dir: str) -> None:
    ckpt = {"epoch": epoch, "model_state_dict": model_state_dict, "optimizer_state_dict": optimizer_state_dict, "scheduler_state_dict": scheduler_state_dict,
            "grad_scaler_state_dict": grad_scaler_state_dict, "loss_info": loss_info, "hist_scores": hist_scores, "best_scores": best_scores}
