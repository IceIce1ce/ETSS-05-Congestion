...
def evaluate(model: nn.Module, data_loader: DataLoader, sliding_window: bool = False, window_size: Optional[int] = None, stride: Optional[int] = None) -> Dict[str, float]:
    model.eval()
    ...
    for image, target_points, _ in tqdm(data_loader):
        image = image.cuda() # [1, 3, 2304, 3072], [[240, 2]]
        ...
        with torch.set_grad_enabled(False):
            if sliding_window:
                ...
                pred_density = model(image) # [1, 1, 288, 384]
            pred_counts.append(pred_density.sum(dim=(1, 2, 3)).cpu().numpy().tolist())
    pred_counts = np.array([item for sublist in pred_counts for item in sublist]) # [500]
    target_counts = np.array([item for sublist in target_counts for item in sublist]) # [500]
