...
class DACELoss(nn.Module):
    ...
    def _bin_count(self, density_map: Tensor) -> Tensor: # [8, 1, 56, 56]
        class_map = torch.zeros_like(density_map, dtype=torch.long) # [8, 1, 56, 56]
        for idx, (low, high) in enumerate(self.bins):
            mask = (density_map >= low) & (density_map <= high) # [8, 1, 56, 56]
            class_map[mask] = idx
        return class_map.squeeze(1) # [8, 56, 56]
    ...
    def forward(self, pred_class: Tensor, pred_density: Tensor, target_density: Tensor, target_points: List[Tensor]) -> Tuple[Tensor, Dict[str, Tensor]]:
        target_density = _reshape_density(target_density, reduction=self.reduction) if target_density.shape[-2:] != pred_density.shape[-2:] else target_density # [8, 1, 56, 56]
        ...
        target_class = self._bin_count(target_density) # [8, 56, 56]
        ...
        if self.use_dm_loss:
            ...
            loss_info = {"ce_loss": cross_entropy_loss.detach(), f"{self.count_loss}_loss": count_loss.detach()}
