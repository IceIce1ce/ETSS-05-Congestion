...
class OTLoss(nn.Module):
    def __init__(self, input_size: int, reduction: int, norm_cood: bool, num_of_iter_in_ot: int = 100, reg: float = 10.0) -> None:
        super().__init__()
...
class DMLoss(nn.Module):
    def __init__(self, input_size: int, reduction: int, norm_cood: bool = False, weight_ot: float = 0.1, weight_tv: float = 0.01, **kwargs: Any) -> None:
        super().__init__()
    ...
    def forward(self, pred_density: Tensor, target_density: Tensor, target_points: List[Tensor]) -> Tuple[Tensor, Dict[str, Tensor]]:
        target_density = _reshape_density(target_density, reduction=self.ot_loss.reduction) if target_density.shape[-2:] != pred_density.shape[-2:] else target_density # [8, 1, 56, 56]
        ...
        pred_count = pred_density.view(pred_density.shape[0], -1).sum(dim=1) # [8]
        normed_pred_density = pred_density / (pred_count.view(-1, 1, 1, 1) + EPS) # [8, 1, 56, 56]
        target_count = torch.tensor([len(p) for p in target_points], dtype=torch.float32).to(target_density.device) # [8]
        normed_target_density = target_density / (target_count.view(-1, 1, 1, 1) + EPS) # [8, 1, 56, 56]
        ...
        loss_info = {"loss": loss.detach(), "ot_loss": ot_loss.detach(), "tv_loss": tv_loss.detach(), "count_loss": count_loss.detach()}
