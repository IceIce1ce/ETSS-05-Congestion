...
def get_model(backbone: str, input_size: int, reduction: int, bins: Optional[List[Tuple[float, float]]] = None, anchor_points: Optional[List[float]] = None, **kwargs: Any) -> Union[Regressor, Classifier, CLIP_EBC]:
    ...
    if "clip" in backbone:
        ...
        return _regressor(backbone=backbone, input_size=input_size, reduction=reduction)
    else:
        ...
        return _classifier(backbone=backbone, input_size=input_size, reduction=reduction, bins=bins, anchor_points=anchor_points)
