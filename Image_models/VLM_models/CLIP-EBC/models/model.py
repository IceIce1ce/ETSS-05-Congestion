...
class Regressor(nn.Module):
    def __init__(self, backbone: nn.Module) -> None:
        ...
        self.regressor = nn.Sequential(nn.Conv2d(backbone.channels, 1, kernel_size=1), nn.ReLU(inplace=True))
        self.regressor.apply(_init_weights)
        self.bins = None
        self.anchor_points = None

    def forward(self, x: Tensor) -> Tensor: # [8, 3, 448, 448]
        x = self.backbone(x) # [8, 64, 56, 56]
        x = self.regressor(x) # [8, 1, 56, 56]
        return x

class Classifier(nn.Module):
    def __init__(self, backbone: nn.Module, bins: List[Tuple[float, float]], anchor_points: List[float]) -> None:
        super().__init__()
        self.backbone = backbone
        self.reduction = backbone.reduction
        ...
        if backbone.channels > 512:
            self.classifier = nn.Sequential(nn.Conv2d(backbone.channels, 512, kernel_size=1), nn.ReLU(inplace=True),
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor, Tensor]]: # [8, 3, 448, 448]
        x = self.backbone(x) # [8, 128, 56, 56]
        x = self.classifier(x) # [8, 5, 56, 56]
        probs = x.softmax(dim=1) # [8, 5, 56, 56]
...
def _regressor(backbone: str, input_size: int, reduction: int) -> Regressor:
    backbone = _get_backbone(backbone.lower(), input_size, reduction)
    return Regressor(backbone())

def _classifier(backbone: nn.Module, input_size: int, reduction: int, bins: List[Tuple[float, float]], anchor_points: List[float]) -> Classifier:
    backbone = _get_backbone(backbone.lower(), input_size, reduction)
