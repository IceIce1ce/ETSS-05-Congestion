...
class ContextualModule(nn.Module):
    def __init__(self, in_channels: int, out_channels: int = 512, sizes: List[int] = [1, 2, 3, 6]) -> None:
        super().__init__()
    ...
    def __make_scale__(self, channels: int, size: int) -> nn.Module:
        return nn.Sequential(nn.AdaptiveAvgPool2d(output_size=(size, size)), nn.Conv2d(channels, channels, kernel_size=1, bias=False))

    def forward(self, feature: Tensor) -> Tensor: # [8, 512, 56, 56]
        h, w = feature.shape[-2:]
        ...
        overall_features = torch.cat([multi_scales, feature], dim=1) # [8, 1024, 56, 56]
        overall_features = self.bottleneck(overall_features) # [8, 512, 56, 56]
        overall_features = self.relu(overall_features) # [8, 512, 56, 56]
        return overall_features

class CANNet(nn.Module):
    def __init__(self, csrnet: CSRNet, sizes: List[int] = [1, 2, 3, 6], reduction: Optional[int] = 8) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor: # [8, 3, 448, 448]
        x = self.features(x) # [8, 512, 56, 56]
        x = self.context(x) # [8, 512, 56, 56]
