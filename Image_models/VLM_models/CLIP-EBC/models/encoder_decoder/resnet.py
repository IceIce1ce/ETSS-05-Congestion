...
model_configs = {"resnet18.tv_in1k": {"decoder_channels": [512, 256, 128]}, "resnet34.tv_in1k": {"decoder_channels": [512, 256, 128]},
                 "resnet50.tv_in1k": {"decoder_channels": [512, 256, 256, 128]}, "resnet101.tv_in1k": {"decoder_channels": [512, 512, 256, 256, 128]},
                 "resnet152.tv_in1k": {"decoder_channels": [512, 512, 512, 256, 256, 128]}}

class ResNet(nn.Module):
    def __init__(self, decoder_block: Union[BasicBlock, Bottleneck], backbone: str = "resnet34.tv_in1k", reduction: Optional[int] = None) -> None:
        super().__init__()
        ...
        self.decoder = make_resnet_layers(block=decoder_block, cfg=decoder_channels, in_channels=encoder_out_channels, dilation=1, expansion=1)
    ...
    def forward(self, x: Tensor) -> Tensor: # [8, 3, 448, 448]
        x = self.encoder(x)[-1] # [8, 512, 28, 28]
        if self.encoder_reduction != self.reduction:
            x = F.interpolate(x, scale_factor=self.encoder_reduction / self.reduction, mode="bilinear") # [8, 512, 56, 56]
        x = self.decoder(x) # [8, 128, 56, 56]
        return x
