...
class CLIP_EBC(nn.Module):
    def __init__(self, backbone: str, bins: List[Tuple[float, float]], anchor_points: List[float], reduction: Optional[int] = None, freeze_text_encoder: bool = True,
                 prompt_type: str = "number", input_size: Optional[int] = None, num_vpt: Optional[int] = None, deep_vpt: Optional[bool] = None, vpt_drop: Optional[float] = None,
                 decoder_block: Optional[nn.Module] = None, decoder_cfg: Optional[List[Union[str, int]]] = None) -> None:
        super().__init__()
    ...
    def _extract_text_features(self) -> None:
        with torch.no_grad():
            self.text_features = self.text_encoder(self.text_prompts) # [5, 512]
    ...
    def _prepare_vpt(self, layer: int, batch_size: int, device: torch.device) -> Tensor:
        ...
        vpt = getattr(self, f"vpt_{layer}").to(device) # [32, 768]
        vpt = vpt.unsqueeze(0).expand(batch_size, -1, -1) # [8, 32, 768]
        vpt = getattr(self, f"vpt_drop_{layer}")(vpt) # [8, 32, 768]
        vpt = vpt.permute(1, 0, 2) # [32, 8, 768]
        assert vpt.shape[1] == batch_size, f"Expected the VPT to have the shape [L_vis B C], got {vpt.shape}."
        return vpt

    def _forward_vpt(self, x: Tensor) -> Tuple[Tensor]: # [8, 3, 448, 448]
        device = x.device
        ...
        image_features = torch.cat([self.image_encoder.class_embedding + torch.zeros(batch_size, 1, image_features.shape[-1], dtype=image_features.dtype, device=device), image_features], dim=1)
        ...
        for idx in range(self.image_encoder_depth):
            image_features = torch.cat([image_features[:1, :, :], vpt, image_features[1:, :, :]], dim=0)
            ...
            image_features = torch.cat([image_features[:1, :, :], image_features[(self.num_vpt + 1):, :, :]], dim=0)
        image_features = image_features.permute(1, 0, 2)
        image_features = self.image_encoder.ln_post(image_features)
        image_features = image_features[:, 1:, :].permute(0, 2, 1)
        image_features = image_features.reshape(batch_size, -1, num_h_patches, num_w_patches) # [8, 768, 28, 28]
        return image_features

    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor, Tensor]]: # [8, 3, 448, 448]
        device = x.device
        ...
        text_features = self.text_encoder(self.text_prompts.to(device)) if self.text_features is None else self.text_features.to(device)
        image_features = F.normalize(image_features, p=2, dim=-1)
        text_features = F.normalize(text_features, p=2, dim=-1)
        logit_scale = self.logit_scale.exp()
        logits = logit_scale * image_features @ text_features.t()
        logits = logits.permute(0, 3, 1, 2) # [8, 5, 56, 56]
        probs = logits.softmax(dim=1)
        exp = (probs * self.anchor_points.to(x.device)).sum(dim=1, keepdim=True) # [8, 1, 56, 56]
        if self.training:
            return logits, exp
        else:
            return exp

def _clip_ebc(backbone: str, bins: List[Tuple[float, float]], anchor_points: List[float], reduction: Optional[int] = None, freeze_text_encoder: bool = True, prompt_type: str = "number",
              input_size: Optional[int] = None, num_vpt: Optional[int] = None, deep_vpt: Optional[bool] = None, vpt_drop: Optional[float] = None,
              decoder_block: Optional[nn.Module] = None, decoder_cfg: Optional[List[Union[str, int]]] = None) -> CLIP_EBC:
    if backbone in resnet_backbones:
        ...
        if decoder_cfg is None:
            if backbone == "vit_b_16":
                ...
                decoder_cfg = [1024]
    return CLIP_EBC(backbone=backbone, bins=bins, anchor_points=anchor_points, reduction=reduction, freeze_text_encoder=freeze_text_encoder, prompt_type=prompt_type,
                    input_size=input_size, num_vpt=num_vpt, deep_vpt=deep_vpt, vpt_drop=vpt_drop, decoder_block=decoder_block, decoder_cfg=decoder_cfg)