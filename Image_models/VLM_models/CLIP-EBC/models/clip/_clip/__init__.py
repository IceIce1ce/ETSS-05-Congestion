...
clip_model_names = ["clip_resnet50", "clip_resnet101", "clip_resnet50x4", "clip_resnet50x16", "clip_resnet50x64", "clip_vit_b_32", "clip_vit_b_16", "clip_vit_l_14", "clip_vit_l_14_336px"]
...
def _clip(name: str, input_size: Optional[Union[int, Tuple[int, int]]] = None) -> CLIP:
    with open(os.path.join(curr_dir, "configs", f"clip_{name}.json"), "r") as f:
        config = json.load(f)
    model = CLIP(embed_dim=config["embed_dim"], image_resolution=config["image_resolution"], vision_layers=config["vision_layers"], vision_width=config["vision_width"], vision_patch_size=config["vision_patch_size"],
                 context_length=config["context_length"], vocab_size=config["vocab_size"], transformer_width=config["transformer_width"], transformer_heads=config["transformer_heads"], transformer_layers=config["transformer_layers"])
...
def _resnet(name: str, reduction: int = 32, features_only: bool = False, out_indices: Optional[Tuple[int, ...]] = None, **kwargs: Any) -> ModifiedResNet:
    with open(os.path.join(curr_dir, "configs", f"clip_image_encoder_{name}.json"), "r") as f:
        config = json.load(f)
    model = ModifiedResNet(layers=config["vision_layers"], output_dim=config["embed_dim"], input_resolution=config["image_resolution"], width=config["vision_width"], heads=config["vision_heads"],
                           features_only=features_only, out_indices=out_indices, reduction=reduction)
...
def _vit(name: str, features_only: bool = False, input_size: Optional[Union[int, Tuple[int, int]]] = None, **kwargs: Any) -> VisionTransformer:
    with open(os.path.join(curr_dir, "configs", f"clip_image_encoder_{name}.json"), "r") as f:
        config = json.load(f)
    model = VisionTransformer(input_resolution=config["image_resolution"], patch_size=config["vision_patch_size"], output_dim=config["embed_dim"], width=config["vision_width"],
                              layers=config["vision_layers"], heads=config["vision_heads"], features_only=features_only)
    state_dict = torch.load(os.path.join(curr_dir, "weights", f"clip_image_encoder_{name}.pth"), map_location="cpu")
    missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
    if len(missing_keys) > 0 or len(unexpected_keys) > 0:
        print(f"Missing keys: {missing_keys}")
...
def _text_encoder(name: str) -> CLIPTextEncoder:
    with open(os.path.join(curr_dir, "configs", f"clip_text_encoder_{name}.json"), "r") as f:
        config = json.load(f)
    model = CLIPTextEncoder(embed_dim=config["embed_dim"], context_length=config["context_length"], vocab_size=config["vocab_size"], transformer_width=config["transformer_width"],
...
__all__ = ["tokenize", "transform", "resnet50_clip", "resnet101_clip", "resnet50x4_clip", "resnet50x16_clip", "resnet50x64_clip", "vit_b_32_clip", "vit_b_16_clip", "vit_l_14_clip",
           "vit_l_14_336px_clip", "resnet50_img", "resnet101_img", "resnet50x4_img", "resnet50x16_img", "resnet50x64_img", "vit_b_32_img", "vit_b_16_img", "vit_l_14_img",
           "vit_l_14_336px_img", "resnet50_txt", "resnet101_txt", "resnet50x4_txt", "resnet50x16_txt", "resnet50x64_txt", "vit_b_32_txt", "vit_b_16_txt", "vit_l_14_txt", "vit_l_14_336px_txt"]