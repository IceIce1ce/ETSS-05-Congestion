...
model_name_map = {"RN50": "resnet50", "RN101": "resnet101", "RN50x4": "resnet50x4", "RN50x16": "resnet50x16", "RN50x64": "resnet50x64", "ViT-B/32": "vit_b_32",
                  "ViT-B/16": "vit_b_16", "ViT-L/14": "vit_l_14", "ViT-L/14@336px": "vit_l_14_336px"}
...
def prepare() -> None:
    ...
    for model_name in tqdm(["RN50", "RN101", "RN50x4", "RN50x16", "RN50x64", "ViT-B/32", "ViT-B/16", "ViT-L/14", "ViT-L/14@336px"]):
        ...
        model_config = {"embed_dim": model.embed_dim, "image_resolution": model.image_resolution, "vision_layers": model.vision_layers, "vision_width": model.vision_width,
                        "vision_patch_size": model.vision_patch_size, "context_length": model.context_length, "vocab_size": model.vocab_size, "transformer_width": model.transformer_width,
                        "transformer_heads": model.transformer_heads, "transformer_layers": model.transformer_layers}
        image_encoder_config = {"embed_dim": model.embed_dim, "image_resolution": model.image_resolution, "vision_layers": model.vision_layers, "vision_width": model.vision_width,
                                "vision_patch_size": model.vision_patch_size, "vision_heads": model.vision_heads}
        text_encoder_config = {"embed_dim": model.embed_dim, "context_length": model.context_length, "vocab_size": model.vocab_size, "transformer_width": model.transformer_width,
                               "transformer_heads": model.transformer_heads, "transformer_layers": model.transformer_layers}
