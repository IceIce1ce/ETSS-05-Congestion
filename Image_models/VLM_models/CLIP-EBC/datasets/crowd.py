...
available_datasets = ["shanghaitech_a", "sha", "shanghaitech_b", "shb", "ucf_qnrf", "qnrf", "ucf-qnrf", "nwpu", "nwpu_crowd", "nwpu-crowd", "jhu", "jhu_crowd", "jhu_crowd_v2"]
...
class Crowd(Dataset):
    def __init__(self, dataset: str, split: str, transforms: Optional[Callable] = None, sigma: Optional[float] = None, return_filename: bool = False, num_crops: int = 1) -> None:
        assert dataset.lower() in available_datasets, f"Dataset {dataset} is not available."
    ...
    def __getitem__(self, idx: int) -> Union[Tuple[Tensor, Tensor, Tensor], Tuple[Tensor, Tensor, Tensor, str]]:
        ...
        if self.image_type == "npy":
            ...
            image = self.to_tensor(image) # [3, 1088, 1632]
        with open(label_path, "rb") as f:
            label = np.load(f)
        label = torch.from_numpy(label).float() # [137, 2]
        ...
        if idx in self.indices:
            density_maps = torch.stack([generate_density_map(label, image.shape[-2], image.shape[-1], sigma=self.sigma) for image, label in zip(images, labels)], 0) # [1, 1, 448, 448]
        else:
            labels = None
            density_maps = None
        image_names = [image_name] * len(images)
        images = torch.stack(images, 0) # [1, 3, 448, 448]
...
class NWPUTest(Dataset):
    def __init__(self, transforms: Optional[Callable] = None, sigma: Optional[float] = None, return_filename: bool = False) -> None:
