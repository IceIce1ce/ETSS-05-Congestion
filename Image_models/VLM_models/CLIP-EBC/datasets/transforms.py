...
def _crop(image: Tensor, label: Tensor, top: int, left: int, height: int, width: int) -> Tuple[Tensor, Tensor]:
...
def _resize(image: Tensor, label: Tensor, height: int, width: int) -> Tuple[Tensor, Tensor]:
...
class RandomCrop(object):
    ...
    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        image_height, image_width = image.shape[-2:]
        assert crop_height <= image_height and crop_width <= image_width, f"crop size should be no larger than image size, got crop size {self.size} and image size {image.shape}."
...
class Resize2Multiple(object):
    def __init__(self, window_size: Tuple[int, int], stride: Tuple[int, int]) -> None:
...
class ZeroPad2Multiple(object):
    ...
    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if new_height == image_height and new_width == image_width:
            ...
            return TF.pad(image, (0, 0, pad_width, pad_height), fill=0), label

class RandomResizedCrop(object):
    def __init__(self, size: Tuple[int, int], scale: Tuple[float, float] = (0.75, 1.25)) -> None:
        self.size = size
        self.scale = scale
        assert len(self.size) == 2, f"size should be a tuple (h, w), got {self.size}."
    ...
    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if crop_height <= in_height and crop_width <= in_width:
            top = torch.randint(0, in_height - crop_height + 1, (1,)).item()
            left = torch.randint(0, in_width - crop_width + 1, (1,)).item()
        else:
            ratio = max(crop_height / in_height, crop_width / in_width)
            resize_height, resize_width = int(in_height * ratio) + 1, int(in_width * ratio) + 1
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        if torch.rand(1) < self.p:
            ...
            if len(label) > 0:
                label[:, 0] = image.shape[-1] - 1 - label[:, 0]
                label[:, 0] = label[:, 0].clamp(min=0, max=image.shape[-1] - 1)
        return image, label

class ColorJitter(object):
    def __init__(self, brightness: Union[float, Tuple[float, float]] = 0.4, contrast: Union[float, Tuple[float, float]] = 0.4, saturation: Union[float, Tuple[float, float]] = 0.4,
                 hue: Union[float, Tuple[float, float]] = 0.2) -> None:
...
class PepperSaltNoise(object):
    ...
    def __call__(self, image: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        image = torch.where(noise < self.saltiness, 1., image)
        image = torch.where(noise > 1 - self.spiciness, 0., image)
        return image, label