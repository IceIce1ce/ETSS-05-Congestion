...
def _generate_random_indices(total_size: int, out_dir: str) -> None:
...
def _resize_and_save(image: np.ndarray, name: str, image_dst_dir: str, generate_npy: bool, label: Optional[np.ndarray] = None, label_dst_dir: Optional[str] = None,
                     min_size: Optional[int] = None, max_size: Optional[int] = None) -> None:
    ...
    if generate_npy:
        ...
        image_npy = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image_npy = np.transpose(image_npy, (2, 0, 1))
...
def _shanghaitech(data_src_dir: str, data_dst_dir: str, min_size: int, max_size: int, generate_npy: bool = False) -> None:
    for split in ["train", "val"]:
        ...
        for i, (image_src_path, label_src_path) in tqdm(enumerate(zip(image_src_paths, label_src_paths)), total=len(image_src_paths)):
            ...
            _resize_and_save(image=image, label=label, name=name, image_dst_dir=image_dst_dir, label_dst_dir=label_dst_dir, generate_npy=generate_npy, min_size=min_size, max_size=max_size)
        if split == "train":
...
def _nwpu(data_src_dir: str, data_dst_dir: str, min_size: int, max_size: int, generate_npy: bool = False) -> None:
    ...
    for image_src_path in tqdm(image_src_paths):
        image_id = os.path.basename(image_src_path).split(".")[0]
        image = cv2.imread(image_src_path)
        _resize_and_save(image=image, label=None, name=image_id, image_dst_dir=image_dst_dir, label_dst_dir=None, generate_npy=generate_npy, min_size=min_size, max_size=max_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type_dataset", type=str, choices=["nwpu", "ucf_qnrf", "jhu", "shanghaitech_a", "shanghaitech_b"], required=True)
    parser.add_argument("--src_dir", type=str, required=True)
    parser.add_argument("--dst_dir", type=str, required=True)
    parser.add_argument("--min_size", type=int, default=256)
    parser.add_argument("--max_size", type=int, default=None)
    parser.add_argument("--generate_npy", action="store_true") # generate .npy files for images
    ...
    _preprocess(dataset=args.type_dataset, data_src_dir=args.src_dir, data_dst_dir=args.dst_dir, min_size=args.min_size, max_size=args.max_size, generate_npy=args.generate_npy)