...
def cf_nerf_collate(batch: Any, extra_mappings: Union[Dict[type, Callable], None] = None) -> Any:
    r"""
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if elem_type.__name__ in ("ndarray", "memmap"):
            ...
            return cf_nerf_collate([torch.as_tensor(b) for b in batch], extra_mappings=extra_mappings)
        ...
        try:
            return elem_type(
                {key: cf_nerf_collate([d[key] for d in batch], extra_mappings=extra_mappings) for key in elem}
            )
        except TypeError:
            # The mapping type may not support `__init__(iterable)`.
            return {key: cf_nerf_collate([d[key] for d in batch], extra_mappings=extra_mappings) for key in elem}
    elif isinstance(elem, tuple) and hasattr(elem, "_fields"):  # namedtuple
        return elem_type(*(cf_nerf_collate(samples, extra_mappings=extra_mappings) for samples in zip(*batch)))
        ...
        if isinstance(elem, tuple):
            return [
                cf_nerf_collate(samples, extra_mappings=extra_mappings) for samples in transposed
            ]  # Backwards compatibility.
        else:
            try:
                return elem_type([cf_nerf_collate(samples, extra_mappings=extra_mappings) for samples in transposed])
            except TypeError:
                # The sequence type may not support `__init__(iterable)` (e.g., `range`).
                return [cf_nerf_collate(samples, extra_mappings=extra_mappings) for samples in transposed]
    ...
    elif isinstance(elem, type(KDTree(torch.asarray([[1, 2], [1, 2]])))):
        return {idx: batch[idx] for idx in range(len(batch))}
