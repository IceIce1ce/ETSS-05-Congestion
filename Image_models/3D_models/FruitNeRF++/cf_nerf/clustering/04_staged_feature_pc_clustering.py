...
def cluster_point_cloud(input_vector, input_xyz_vector, cluster_type="SC"):
    t1 = time.time()
    if cluster_type == 'DBSCAN':
        clustering = DBSCAN(eps=0.001,
                            min_samples=50,
                            metric="cosine").fit(input_vector)
    elif cluster_type == 'HDBSCAN':
        clustering = hdbscan.HDBSCAN(min_cluster_size=10,
                                     min_samples=5,
                                     prediction_data=False,
                                     allow_single_cluster=False).fit(input_vector)
    elif cluster_type == 'KMeans':
        clustering = KMeans(n_clusters=8, random_state=0, n_init="auto").fit(input_vector)

    elif cluster_type == 'SC':
        clustering = SpectralClustering(n_clusters=2,
                                        assign_labels='discretize',
                                        random_state=0).fit(input_vector)
    ...
    pcd_clustered.points = o3d.utility.Vector3dVector(input_xyz_vector)
    ...
    pcd_clustered.colors = o3d.utility.Vector3dVector(color)
    #o3d.visualization.draw_geometries([pcd])

    return clustering, pcd_clustered


instance_pc_path = Path(
    "/home/se86kimy/Dropbox/05_productive/01_code/15_ContrastiveFruitNeRF/CF-NeRF/debug/outputs/01_apple_tree_2048x2048_#300_test/cf-nerf-small/2024-06-19_165150/cf-nerf-small/instance_feature_vec.ply")
...
#### Point Coordinate clustering
clustering_type = 'DBSCAN'

t1 = time.time()
if clustering_type == 'DBSCAN':
    clustering = DBSCAN(eps=0.0001,
                        min_samples=50,
...
for i in num_detected_clusters:
    if i == -1:
        continue
    feature_cluster_curr = cluster_vectors_normalized[np.where(clustering.labels_ == i)]
    points_curr = xyz_vectors[np.where(clustering.labels_ == i)]

    pcd = cluster_point_cloud(feature_cluster_curr, points_curr)
