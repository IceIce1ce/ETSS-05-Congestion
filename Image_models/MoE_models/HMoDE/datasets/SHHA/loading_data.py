...
def get_min_size(batch):
    ...
    for i_sample in batch:
        ...
        if wd < min_wd:
            min_wd = wd
    return min_ht // 32 * 32, min_wd // 32 * 32
...
def loading_data():
    ...
    train_main_transform = own_transforms.Compose([own_transforms.RandomHorizontallyFlip()])
    img_transform = standard_transforms.Compose([standard_transforms.ToTensor(), standard_transforms.Normalize(*mean_std)])
    gt_transform = standard_transforms.Compose([own_transforms.GTScaleDown(factor), own_transforms.LabelNormalize(log_para)])
    train_set = SHHA(cfg_data.DATA_PATH+'/train_data', main_transform=train_main_transform, img_transform=img_transform, gt_transform=gt_transform, data_augment=cfg_data.AUGMENT)
    train_loader =None
    if cfg_data.TRAIN_BATCH_SIZE == 1:
        ...
        train_loader = DataLoader(train_set, batch_size=cfg_data.TRAIN_BATCH_SIZE, num_workers=32, collate_fn=SHHA_collate, shuffle=True, drop_last=True)
    val_set = SHHA(cfg_data.DATA_PATH + '/test_data', main_transform=None, img_transform=img_transform, gt_transform=gt_transform, data_augment=1)
    val_loader = DataLoader(val_set, batch_size=cfg_data.VAL_BATCH_SIZE, num_workers=32, shuffle=True, drop_last=False)
