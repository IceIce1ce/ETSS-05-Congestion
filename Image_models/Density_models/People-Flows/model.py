...
class ContextualModule(nn.Module):
    ...
    def forward(self, feats): # [1, 512, 45, 80]
        h, w = feats.size(2), feats.size(3)
        multi_scales = [F.upsample(input=stage(feats), size=(h, w), mode='bilinear') for stage in self.scales]
        weights = [self.__make_weight(feats,scale_feature) for scale_feature in multi_scales]
        overall_features = [(multi_scales[0] * weights[0] + multi_scales[1] * weights[1] + multi_scales[2] * weights[2] + multi_scales[3] * weights[3]) /
                            (weights[0] + weights[1] + weights[2] + weights[3])] + [feats]
...
class CANNet2s(nn.Module):
    ...
    def forward(self, x_prev, x): # [1, 3, 360, 640], [1, 3, 360, 640]
        x_prev = self.frontend(x_prev) # [1, 512, 45, 80]
        x = self.frontend(x) # [1, 512, 45, 80]
        x_prev = self.context(x_prev) # [1, 512, 45, 80]
        x = self.context(x) # [1, 512, 45, 80]
        x = torch.cat((x_prev,x),1) # [1, 1024, 45, 80]
        x = self.backend(x) # [1, 64, 45, 80]
        x = self.output_layer(x) # [1, 10, 45, 80]
