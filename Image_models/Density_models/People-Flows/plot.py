...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_dir', type=str, default='saved_vis')
    ...
    for i in range(len(img_paths)):
        ...
        gt_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_gt.jpg')
        density_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_pred.jpg')
        flow_1_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_1.jpg')
        flow_2_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_2.jpg')
        flow_3_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_3.jpg')
        flow_4_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_4.jpg')
        flow_5_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_5.jpg')
        flow_6_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_6.jpg')
        flow_7_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_7.jpg')
        flow_8_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_8.jpg')
        flow_9_path = os.path.join(args.vis_dir, base_name).replace('.jpg','_'+folder_name+'_flow_9.jpg')
        pred = cv2.resize(overall,(overall.shape[1]*8,overall.shape[0]*8),interpolation = cv2.INTER_CUBIC)/64.0 # [360, 640]
        prev_flow = prev_flow.data.cpu().numpy()[0] # [10, 45, 80]
