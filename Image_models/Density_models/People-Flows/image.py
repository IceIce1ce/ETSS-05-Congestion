...
def load_data(img_path):
    ...
    gt_file.close()
    target = cv2.resize(target,(int(target.shape[1] / 8),int(target.shape[0] / 8)),interpolation=cv2.INTER_CUBIC) * 64 # [45, 80]
    prev_gt_file = h5py.File(prev_gt_path)
    prev_target = np.asarray(prev_gt_file['density']) # [360, 640]
    prev_gt_file.close()
    prev_target = cv2.resize(prev_target, (int(prev_target.shape[1] / 8), int(prev_target.shape[0] / 8)),interpolation=cv2.INTER_CUBIC) * 64 # [45, 80]
    post_gt_file = h5py.File(post_gt_path)
    post_target = np.asarray(post_gt_file['density']) # [360, 640]
    post_gt_file.close()
    post_target = cv2.resize(post_target, (int(post_target.shape[1] / 8), int(post_target.shape[0] / 8)),interpolation=cv2.INTER_CUBIC) * 64 # [45, 80]
