...
def pre_data(train_list, args, train):
    ...
    for j in range(len(train_list)):
        ...
        img, fidt_map, kpoint = load_data_fidt(Img_path) # [768, 1024, 3], [768, 1024], [768, 1024]
...
def train(Pre_data, model, criterion, optimizer, epoch, args):
    ...
    # train loader
    train_loader = torch.utils.data.DataLoader(dataset.listDataset(Pre_data, args['output_dir'], transform=transforms.Compose([transforms.ToTensor(),
                                               transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]), train=True, args=args),
    ...
    for i, (fname, img, fidt_map, kpoint) in enumerate(train_loader):
        ...
        fidt_map = fidt_map.type(torch.FloatTensor).unsqueeze(1).cuda() # [16, 1, 256, 256]
        d6 = model(img) # [16, 1, 256, 256]
...
def validate(Pre_data, model, args):
    ...
    for i, (fname, img, fidt_map, kpoint) in enumerate(test_loader):
        ...
        with torch.no_grad():
            d6 = model(img) # [1, 1, 704, 1024]
            count, pred_kpoint, f_loc = LMDS_counting(d6, i + 1, f_loc, args)
            point_map = generate_point_map(pred_kpoint, f_loc, rate=1) # [704, 1024, 3]
            if args['visual'] == True:
                if not os.path.exists(os.path.join(args['output_dir'], 'vis_box')):
                    os.makedirs(os.path.join(args['output_dir'], 'vis_box'))
                ori_img, box_img = generate_bounding_boxes(pred_kpoint, fname, args) # [704, 1024, 3], [704, 1024, 3]
                show_fidt = show_map(d6.data.cpu().numpy()) # [704, 1024, 3]
                gt_show = show_map(fidt_map.data.cpu().numpy()) # [704, 1024, 3]
                res = np.hstack((ori_img, gt_show, show_fidt, point_map, box_img)) # [704, 5120, 3]
...
def LMDS_counting(input, w_fname, f_loc, args):
    ...
    input = keep * input # [1, 1, 704, 1024]
    input[input < 100.0 / 255.0 * input_max] = 0
    input[input > 0] = 1
    if input_max < 0.1:
        input = input * 0
    count = int(torch.sum(input).item())
    kpoint = input.data.squeeze(0).squeeze(0).cpu().numpy() # [704, 1024]
    if f_loc != None:
        f_loc.write('{} {} '.format(w_fname, count))
    return count, kpoint, f_loc

def generate_point_map(kpoint, f_loc, rate=1):
    pred_coor = np.nonzero(kpoint) # [1923]
    point_map = np.zeros((int(kpoint.shape[0] * rate), int(kpoint.shape[1] * rate), 3), dtype="uint8") + 255 # [704, 1024, 3]
    coord_list = []
...
def main(args):
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        if epoch % 10 == 0 and epoch >= 200:
            ...
            print('Epoch: [{}/{}], Best MAE: {:.2f}'.format(epoch + 1, args['epochs'], args['best_pred']))
