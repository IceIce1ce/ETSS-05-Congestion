...
class VGG16_frontend(nn.Module):
    def __init__(self,block_num=5,decode_num=0,load_weights=True,bn=False,IF_freeze_bn=False):
        ...
        block_dict = [[64, 64, 'M'], [128, 128, 'M'], [256, 256, 256, 'M'], [512, 512, 512,'M'], [512, 512, 512,'M']]
        self.frontend_feat = []
    ...
    def forward(self,x):
        if self.bn: 
            ...
            conv5_feat =x
        feature_map = {'conv1':conv1_feat,'conv2': conv2_feat, 'conv3':conv3_feat,'conv4': conv4_feat, 'conv5': conv5_feat}
        return feature_map
...
class SSDCNet_classify(nn.Module):
    def __init__(self, class_num,label_indice, div_times=2, frontend_name='VGG16', block_num=5, IF_pre_bn=True, IF_freeze_bn=False, load_weights=False, psize=64, pstride=64, parse_method ='maxp', den_for='p'):
        ...
        if self.frontend_name == 'VGG16':
            ...
            for i in range(self.div_times):
                back_end_up.append( up(up_in_ch[i],up_out_ch[i], cat_in_ch[i],cat_out_ch[i]) )
            if self.div_times>0:
                self.back_end_up = nn.Sequential(*back_end_up)
        self.back_end_cls = torch.nn.Sequential(torch.nn.AvgPool2d((2,2),stride=2), torch.nn.Conv2d(512, 512, (1, 1)), torch.nn.ReLU(),
                                                torch.nn.Conv2d(512, class_num, (1, 1)))
        self.back_end_lw_fc = torch.nn.Sequential(torch.nn.AvgPool2d((2,2),stride=2), torch.nn.Conv2d(512, 512, (1, 1)), torch.nn.ReLU(),
                                                  torch.nn.Conv2d(512, 1, (1, 1)))
        self.back_end_denisty = torch.nn.Sequential(torch.nn.Conv2d(512, 512, (1, 1)), torch.nn.ReLU(),
                                                    torch.nn.Conv2d(512, 1, (1, 1)))
    ...
    def forward(self, x): # [1, 3, 704, 1024]
        x = self.front_end(x)
        return x
