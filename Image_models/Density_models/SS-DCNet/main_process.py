...
def main(opt):
    ...
    # test loader
    testset = Countmap_Dataset(img_dir,tar_dir,rgb_dir,transform=transform_test, if_test=True, IF_loadmem=opt['IF_savemem_test'])
    ...
    # model
    net = SSDCNet_classify(class_num,label_indice,div_times=div_times, frontend_name='VGG16',block_num=5, IF_pre_bn=False,IF_freeze_bn=False,load_weights=True, psize=opt['psize'],pstride = opt['pstride'],parse_method ='maxp').cuda()
    ...
    if os.path.exists(mod_path):
        ...
        log_save_path = os.path.join(save_folder,'log-epoch-min[%d]-%s.txt' % (tmp_epoch_num+1,opt['parse_method']) )
