...
class MSSRM(nn.Module):
    def __init__(self, load_weights=False, upscale='x2'):
        ...
        self.reg_layer = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1), nn.ReLU(inplace=True),
                                       ...
                                       nn.Conv2d(128, 1, 1))
        self.conv = nn.Sequential(nn.Conv2d(1280, 1280, kernel_size=1, padding=1))
        if upscale == 'x2':
            self.espcn_part = nn.Sequential(nn.Conv2d(1280, 3 * (4 ** 2), kernel_size=3, padding=3 // 2), nn.PixelShuffle(4))
        elif upscale == 'x4':
            self.espcn_part = nn.Sequential(nn.Conv2d(1280, 3 * (8 ** 2), kernel_size=3, padding=3 // 2), nn.PixelShuffle(8))
        self.pixel2 = nn.Sequential(nn.Conv2d(512, 512 * (2 ** 2), kernel_size=3, padding=3 // 2), nn.PixelShuffle(2))
        self.pixel4 = nn.Sequential(nn.Conv2d(1, 1 * (4 ** 2), kernel_size=3, padding=3 // 2), nn.PixelShuffle(4))
    ...
    def forward(self, x, gt, gt_sr, phase='test'): # [1, 3, 341, 512], [1, 341, 512], None
        pd = (4, 4, 4, 4)
