...
def compute_metrics(dist_matrix, match_matrix, pred_num, sigma, level):
    num_classes = 6
...
def eval_metrics(num_classes, pred_data, gt_data_T):
    if gt_data_T['num'] > 0:
        gt_data = {'num': gt_data_T['num'].numpy().squeeze(), 'points': gt_data_T['points'].numpy().squeeze(), 'sigma': gt_data_T['sigma'].numpy().squeeze(),
                   'level': gt_data_T['level'].numpy().squeeze()}
    else:
    ...
    if gt_data['num'] != 0 and pred_data['num'] != 0:
        ...
        tp_s, fp_s, fn_s, tp_c_s, fn_c_s = compute_metrics(dist_matrix, match_matrix, pred_p.shape[0], sigma_s, level)
        tp_l, fp_l, fn_l, tp_c_l, fn_c_l = compute_metrics(dist_matrix, match_matrix, pred_p.shape[0], sigma_l, level)
