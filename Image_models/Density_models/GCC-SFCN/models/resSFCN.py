...
class resSFCN(nn.Module):
    ...
    def forward(self, x): # [2, 3, 576, 768]
        x = self.frontend(x) # [2, 512, 72, 96]
        x = self.own_reslayer_3(x) # [2, 1024, 72, 96]
        x = self.backend(x) # [2, 64, 72, 96]
        x = self.convDU(x) # [2, 64, 72, 96]
        x = self.convLR(x) # [2, 64, 72, 96]
        x = self.output_layer(x) # [2, 1, 72, 96]
        x = F.upsample(x,scale_factor=8) # [2, 1, 576, 768]
        return x
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x): # [2, 512, 72, 96]
        residual = x
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        ...
        out += residual
        out = self.relu(out) # p2, 1024, 72, 96]
        return out        