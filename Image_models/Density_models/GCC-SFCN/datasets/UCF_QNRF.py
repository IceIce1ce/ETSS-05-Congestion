...
class UCF_QNRF(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.main_transform is not None:
            img, den = self.main_transform(img,den) # [576, 768, 3], [576, 768]
        if self.img_transform is not None:
            img = self.img_transform(img) # [3, 576, 768]
        gt_count = torch.from_numpy(np.array(den)).sum() # [1]
        if self.gt_transform is not None:
            den = self.gt_transform(den) # [576, 768]
