...
class SHTechPartADataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.root_path = opt.input_dir
        if opt.isTrain:
            if opt.type_dataset == 'sha':
                self.img_list_file = os.path.join(self.root_path, "train_sha.txt")
            elif opt.type_dataset == 'shb':
                self.img_list_file = os.path.join(self.root_path, "train_shb.txt")
            else:
                print('This dataset does not exist')
                raise NotImplementedError
        else:
            if opt.type_dataset == 'sha':
                self.img_list_file = os.path.join(self.root_path, "test_sha.txt")
            elif opt.type_dataset == 'shb':
                self.img_list_file = os.path.join(self.root_path, "test_shb.txt")
            else:
                print('This dataset does not exist')
                raise NotImplementedError
        self.img_map = {}
    ...
    def modify_commandline_options(parser):
        parser.add_argument('--limit_shorter_side', default=256, type=int)
    ...
    def get_ada_density_map_gaussian(self, img_shape, points):
        ...
        for ind, pt in enumerate(points):
            ...
            if gt_count > 1:
                ...
                sigma = 0.1 * min(img_shape[0], img_shape[1])
    ...
    def __getitem__(self, idx):
        while True:
            ...
            if crop_ratio > 0:
                ...
                crop_patch_list = [(0, 0), (img_width - crop_size[0], 0), (0, img_height - crop_size[1]), (img_width - crop_size[0], img_height - crop_size[1]), (random.randint(0, img_width - crop_size[0]), random.randint(0, img_height - crop_size[1])),
                                   (random.randint(0, img_width - crop_size[0]), random.randint(0, img_height - crop_size[1])), (random.randint(0, img_width - crop_size[0]), random.randint(0, img_height - crop_size[1])),
                                   (random.randint(0, img_width - crop_size[0]), random.randint(0, img_height - crop_size[1])), (random.randint(0, img_width - crop_size[0]), random.randint(0, img_height - crop_size[1]))]
    ...
    def get_test_imgs(self, img_list):
        ...
        with open(img_list, 'rt') as f_list:
            for line in f_list:
                file_path = os.path.join(self.opt.input_dir, line.strip().split()[0]).strip()
                file_list.append(file_path)
                file_map[file_path] = os.path.join(self.opt.input_dir, line.strip().split()[1]).strip()
        final_img_list = {}
