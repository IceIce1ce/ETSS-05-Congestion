...
class Inception3(nn.Module):
    ...
    def forward(self, x): # [24, 3, 128, 128]
        ...
        density_map3 = density_map3.view(-1, density_map3.size(2), density_map3.size(3)) # [24, 32, 32]
        attention_map = attention_map.view(-1, attention_map.size(2), attention_map.size(3)) # [24, 32, 32]
...
class InceptionA(nn.Module):
    ...
    def forward(self, x): # [24, 192, 64, 64]
        ...
        return torch.cat(outputs, 1) # [24, 256, 64, 64]
...
class InceptionB(nn.Module):
    ...
    def forward(self, x): # [24, 288, 64, 64]
        ...
        return torch.cat(outputs, 1) # [24, 768, 32, 32]
...
class InceptionC(nn.Module):
    ...
    def forward(self, x): # [24, 768, 32, 32]
        branch1x1 = self.branch1x1(x)
...
class InceptionD(nn.Module):
    ...
    def forward(self, x): # [24, 768, 32, 32]
        ...
        return torch.cat(outputs, 1) # [24, 1280, 16, 16]
...
class InceptionE(nn.Module):
    ...
    def forward(self, x): # [24, 1280, 16, 16]
        branch1x1 = self.branch1x1(x)
        branch3x3 = self.branch3x3_1(x)
        ...
        return torch.cat(outputs, 1) # [24, 2048, 16, 16]
...
class BasicConv2d(nn.Module):
    ...
    def forward(self, x): # [24, 3, 128, 128]
        x = self.conv(x)
        x = self.bn(x)
        return F.relu(x, inplace=True) # [24, 32, 64, 64]