...
def train_one_epoch(model: torch.nn.Module, counting_criterion: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer, metric_logger: object,
                    loss_weight: object, epoch, args):
    model.train()
    ...
    for inputs, labels in metric_logger.log_every(data_loader):
        optimizer.zero_grad()
        inputs = inputs.to(args.gpu) # [8, 3, 1024, 1024]
        output_aux_dmap = model(inputs) # [8, 1, 128, 128]
        all_loss = 0
        loss_dict = {}
        for j, dmap in enumerate(output_aux_dmap):
            aux_scale = inputs.shape[-1] / dmap.shape[-1]
            aux_counting_loss, aux_dMaps = counting_criterion(dmap, labels, aux_scale)
...
@torch.no_grad()
def evaluate_counting(model, counting_criterion, data_loader, metric_logger, args):
    model.eval()
    ...
    with torch.no_grad():
        for inputs, labels in metric_logger.log_every(data_loader):
            ...
            inputs = inputs.to(args.gpu) # [1, 3, 1024, 1024]
            output_aux_dmap = model(inputs)[-1] # [1, 128, 128]
            ...
            nae = torch.abs(predict_nums - target_nums).data / (max(1, target_nums.data))
            metrics[f"mae"] = mae
            metrics[f"rmse"] = rmse
            metrics[f"nae"] = nae
