...
class Saver():
    ...
    def save(self, model, optimizer, scheduler, filename, epoch, stats={}):
        if is_main_process():
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'scheduler': scheduler.state_dict(), 'epoch': epoch,
                        'states': stats}, os.path.join(self.save_dir, filename))
    ...
    def save_on_master(self, model, optimizer, scheduler, epoch, stats={}):
        if is_main_process():
            self.save_inter(model, optimizer, scheduler, f"checkpoint{epoch:04}_{self.is_ema}.pth", epoch, stats)
            print(f"save checkpoint{epoch:04}_{self.is_ema}")
            if self.is_ema == "ema":
                if self.save_best and stats.ema_test_stats[self.metric] < self.best_metric:
                    self.best_metric = stats.ema_test_stats[self.metric]
                    self.save(model, optimizer, scheduler, f"best_{self.is_ema}.pth", epoch, stats)
                    print(f"save best ema model,mae:{self.best_metric},epoch:{epoch}")
            else:
                if self.save_best and stats.test_stats[self.metric] < self.best_metric:
                    self.best_metric = stats.test_stats[self.metric]
                    print(f"save best model,{self.metric}:{self.best_metric},epoch:{epoch}")
                    self.save(model, optimizer, scheduler,f"best_{self.is_ema}.pth", epoch, stats)