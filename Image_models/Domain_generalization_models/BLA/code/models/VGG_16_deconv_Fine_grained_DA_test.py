...
from vision import save_keypoints_and_img
...
class Conv2d(nn.Module):
    ...
    def forward(self, x): # [1, 512, 88, 128]
        x = self.conv(x) # [1, 128, 88, 128]
        if self.bn is not None:
            x = self.bn(x) # [1, 128, 88, 128]
        if self.relu is not None:
            x = self.relu(x) # [1, 128, 88, 128]
        return x
...
class BasicDeconv(nn.Module):
    ...
    def forward(self, x): # [1, 128, 88, 128]
        x = self.tconv(x) # [1, 128, 176, 256]
        if self.use_bn:
            x = self.bn(x) # [1, 128, 176, 256]
...
class den_level_Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=512, num_classes=1,opt=None):
        ...
        self.D = nn.Sequential(nn.Conv2d(input_nc, ndf, kernel_size=3, stride=1, padding=1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                               nn.Conv2d(ndf, ndf//2, kernel_size=3, stride=1, padding=1), nn.LeakyReLU(negative_slope=0.2, inplace=True))
...
class VGG_16_deconv_Fine_grained_DA_test(nn.Module):
    def __init__(self, opt):
        ...
        self.den_pred = nn.Sequential(Conv2d(512, 128, 3, same_padding=True, bn=True, NL='relu'), BasicDeconv(128, 128, 2, 2, True),
                                      Conv2d(128, 64, 3, same_padding=True, bn=True, NL='relu'), BasicDeconv(64, 64, 2, 2, True),
                                      Conv2d(64, 32, 3, same_padding=True, bn=True, NL='relu'), BasicDeconv(32, 32, 2, 2, True),
                                      Conv2d(32, 1, 1, same_padding=True, NL=''), nn.ReLU(inplace=True))
    ...
    def inference(self, data, epoch, step, vision_list):
        batch = len(data[0])
        for i in range(batch):
            x = self.backbone(data[0][i].unsqueeze(0).cuda()) # [1, 512, 88, 128]
            den_map = self.den_pred(x) # [1, 1, 704, 1024]
            sum_map = torch.sum(den_map[i].detach().cpu())/self.opt.gt_factor # [1]
