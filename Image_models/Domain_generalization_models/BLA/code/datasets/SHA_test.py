...
class Gaussian(nn.Module):
    ...
    def forward(self, dotmaps): # [1, 1, 320, 480]
        gaussianmaps = self.gkernel(dotmaps) # [1, 1, 320, 480]
        return gaussianmaps
...
class Gaussianlayer(nn.Module):
    ...
    def forward(self, dotmaps): # [1, 1, 320, 480]
        denmaps = self.gaussian(dotmaps) # [1, 1, 320, 480]
        return denmaps
...
class gen_den_map(nn.Module):
    ...
    def forward(self, dot_map): # [1, 1, 320, 480]
        gt_map = self.gs(dot_map) # [1, 1, 320, 480]
        return gt_map
...
class SHA_dataloader(data.Dataset):
    def __init__(self,split,opt):
        ...
        imgfiles = [filename for filename in os.listdir(self.img_root) if os.path.isfile(os.path.join(self.img_root,filename)) and filename.split('.')[1] in ['jpg','png']]
