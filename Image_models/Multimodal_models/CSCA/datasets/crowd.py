...
class Crowd_shanghaiTechRGBD(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.method == 'train':
            ...
            d_path = gt_path.replace('train_gt', 'train_depth').replace('GT', 'DEPTH')
            RGB = cv2.imread(rgb_path)[..., ::-1].copy()
            Depth = scipy.io.loadmat(d_path)['depth'].astype(np.float32)
            ...
            d_path = gt_path.replace('test_bbox_anno', 'test_depth').replace('BBOX', 'DEPTH')
            RGB = cv2.imread(rgb_path)[..., ::-1].copy()
            Depth = scipy.io.loadmat(d_path)['depth'].astype(np.float32)
