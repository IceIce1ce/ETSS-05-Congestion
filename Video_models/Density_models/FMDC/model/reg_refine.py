...
class FlowHead(nn.Module):
    ...
    def forward(self, x): # [2, 128, 48, 64]
        out = self.conv2(self.relu(self.conv1(x))) # [2, 2, 48, 64]
        return out
...
class SepConvGRU(nn.Module):
    ...
    def forward(self, h, x): # [2, 128, 48, 64], [2, 256, 48, 64]
...
class BasicMotionEncoder(nn.Module):
    ...
    def forward(self, flow, corr): # [2, 2, 48, 64]
        ...
        return torch.cat([out, flow], dim=1) # [2, 128, 48, 64]
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, corr_channels=324, hidden_dim=128, context_dim=128, downsample_factor=8, flow_dim=2, bilinear_up=False):
        ...
        if bilinear_up:
            ...
            self.mask = nn.Sequential(nn.Conv2d(hidden_dim, 256, 3, padding=1), nn.ReLU(inplace=True),
                                      nn.Conv2d(256, downsample_factor ** 2 * 9, 1, padding=0))

    def forward(self, net, inp, corr, flow): # [2, 128, 48, 64], [2, 128, 48, 64], [2, 81, 48, 64], [2, 2, 48, 64]
        ...
        if self.mask is not None:
            ...
            mask = None
        return net, mask, delta_flow # [2, 128, 48, 64], [2, 144, 48, 64], [2, 2, 48, 64]