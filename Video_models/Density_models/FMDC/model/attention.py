# https://huggingface.co/stevetod/doduo/blob/main/attention.py
import torch
...
def single_head_split_window_attention_1d(q, k, v, num_splits=1, with_shift=False, h=None, w=None, attn_mask=None):
    assert h is not None and w is not None
    assert q.size(1) == h * w
    b, _, c = q.size()
...
class SelfAttnPropagation(nn.Module):
    ...
    def forward_local_window_attn(self, feature0, flow, local_window_radius=1):
        ...
        flow_window = flow_window.view(b, value_channel, kernel_size ** 2, h, w).permute(0, 3, 4, 2, 1).reshape(b * h * w, kernel_size ** 2, value_channel)
        scores = torch.matmul(feature0_reshape, feature0_window) / (c ** 0.5)
        prob = torch.softmax(scores, dim=-1)
        out = torch.matmul(prob, flow_window).view(b, h, w, value_channel ).permute(0, 3, 1, 2).contiguous()
        return out