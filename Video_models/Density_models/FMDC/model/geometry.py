# https://huggingface.co/stevetod/doduo/blob/main/geometry.py
import torch
...
def generate_window_grid(h_min, h_max, w_min, w_max, len_h, len_w, device=None):
    ...
    x, y = torch.meshgrid([torch.linspace(w_min, w_max, len_w, device=device), torch.linspace(h_min, h_max, len_h, device=device)])
...
def camera_transform(points_ref, extrinsics_ref=None, extrinsics_tgt=None, extrinsics_rel=None):
    ...
    points_tgt = torch.bmm(extrinsics_rel[:, :3, :3], points_ref.view(b, 3, -1)) + extrinsics_rel[:, :3, -1:]
...
def reproject(points_tgt, intrinsics, return_mask=False):
    ...
    if return_mask:
        mask = (pixel_coords[:, 0] >= 0) & (pixel_coords[:, 0] <= (w - 1)) & ( pixel_coords[:, 1] >= 0) & (pixel_coords[:, 1] <= (h - 1))
...
def reproject_coords(depth_ref, intrinsics, extrinsics_ref=None, extrinsics_tgt=None, extrinsics_rel=None, return_mask=False):
    ...
    if return_mask:
        reproj_coords, mask = reproject(points_tgt, intrinsics, return_mask=return_mask)
        return reproj_coords, mask
    reproj_coords = reproject(points_tgt, intrinsics, return_mask=return_mask)
...
def compute_flow_with_depth_pose(depth_ref, intrinsics, extrinsics_ref=None, extrinsics_tgt=None, extrinsics_rel=None, return_mask=False):
    ...
    if return_mask:
        reproj_coords, mask = reproject_coords(depth_ref, intrinsics, extrinsics_ref, extrinsics_tgt, extrinsics_rel=extrinsics_rel, return_mask=return_mask)
        rigid_flow = reproj_coords - coords_init
        return rigid_flow, mask
    reproj_coords = reproject_coords(depth_ref, intrinsics, extrinsics_ref, extrinsics_tgt, extrinsics_rel=extrinsics_rel, return_mask=return_mask)
    rigid_flow = reproj_coords - coords_init
    return rigid_flow