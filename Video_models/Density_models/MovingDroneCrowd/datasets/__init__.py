...
class train_pair_transform(object):
    ...
    def __call__(self, img, target):
        ...
        img, target = check_image(img, target, (self.c_h, self.c_w),  (self.cfg_data.TRAINING_MAX_LONG, self.cfg_data.TRAINING_MAX_SHORT))
        w, h = img.size
...
def createTrainData(datasetname, Dataset, cfg_data, distributed):
    ...
    train_set = Dataset(cfg_data.TRAIN_LST, cfg_data.DATA_PATH, main_transform=resize_transform if datasetname=="UAVVIC" else pair_transform, img_transform=img_transform,
                        datasetname=datasetname, frame_intervals=cfg_data.TRAIN_FRAME_INTERVALS)
    sampler_train = DistributedSampler(train_set) if distributed else None
    train_loader = DataLoader(train_set, batch_size=cfg_data.TRAIN_BATCH_SIZE, sampler=sampler_train, shuffle=False, num_workers=8, collate_fn=collate_fn, pin_memory=True)
...
def createValTestData(datasetname, Dataset, cfg_data, frame_interval, skip_flag, is_main, mode='val'):
    if is_main:
        ...
        data_loader = []
        for scene_name in scene_names:
            sub_dataset = Dataset(scene_name=scene_name, base_path=cfg_data.DATA_PATH, main_transform=main_transform, img_transform=img_transform, interval=frame_interval,
                                  skip_flag=skip_flag, target=target, datasetname = datasetname)
            sub_loader = DataLoader(sub_dataset, batch_size=cfg_data.VAL_BATCH_SIZE, shuffle=False, collate_fn=collate_fn, num_workers=0, pin_memory=True)
