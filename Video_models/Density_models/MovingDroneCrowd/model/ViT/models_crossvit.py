...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, x1, x2): # [2, 256, 48, 64], [2, 256, 48, 64]
        fused = torch.cat([x1, x2], dim=1)
        fused = self.conv1(fused)
        fused = self.bn1(fused)
        fused = self.relu(fused) # [2, 256, 48, 64]
        return fused
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y): # [1, 3072, 256], [1, 3072, 256]
        B, Nx, C = x.shape
        Ny = y.shape[1]
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x, y): # [1, 3072, 256], [1, 3072, 256]
        ...
        x = x + self.drop_path2(self.mlp(self.norm2(x))) # [1, 3072, 256]
        return x