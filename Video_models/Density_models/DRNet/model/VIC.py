...
from model.points_from_den import get_ROI_and_MatchInfo, local_maximum_points
...
class Video_Individual_Counter(nn.Module):
    def __init__(self, cfg, cfg_data):
        ...
        OT_config = {'feature_dim': cfg.FEATURE_DIM, 'sinkhorn_iterations': cfg.sinkhorn_iterations, 'matched_threshold': 0.2}
        ...
        self.get_ROI_and_MatchInfo = get_ROI_and_MatchInfo(self.dataset_cfg.TRAIN_SIZE, self.radius)
    ...
    def KPI_cal(self, match_gt, scores, target_pair):
        ...
        scores[-1, -1] = 0
        max0_idx = scores.max(1).indices
        max1_idx = scores.max(0).indices
        if match_gt['a2b'].size(0) > 0:
            ...
            TP = (b_id[pred_a] == a_id[pred_b]).sum()
        else:
            TP = 0
    ...
    def forward(self, img, target=None):
        ...
        img = torch.stack(img, 0) # [8, 3, 768, 1024]
        img_pair_num = img.size(0) // 2
        assert img.size(0) % 2 == 0
        features, pre_map = self.Extractor(img) # [8, 256, 192, 256], [8, 1, 768, 1024]
        dot_map = torch.zeros_like(pre_map)
        for i, data in enumerate(target):
            points = data['points'].long() # [35, 2]
            dot_map[i, 0, points[:, 1], points[:, 0]] = 1
        gt_den = self.Gaussian(dot_map) # [8, 1, 768, 1024]
        ...
        matched_results = {'matches0': [], 'matches1': [],'matching_scores0': [], 'matching_scores1': [], 'gt_matched': [], 'gt_count_diff': 0, 'pre_count_diff': 0}
        ...
        # second matching
        pre_data = local_maximum_points(pre_map.detach(),self.gaussian_maximum,radius=self.radius) # 4364, [4364, 4], [4364, 5]
        precount_in_batch = [(pre_data['points'][:, 0] == i).sum() for i in range(pre_map.size(0))]
        pre_points = torch.split(pre_data['points'], precount_in_batch) # [966, 4]
        for pair_idx in range(img_pair_num):
            ...
            if (np.array(count_in_pair) > 0).all() and (np.array(count_in_pair) < 4000).all():
                ...
                scores, indices0, indices1, mscores0, mscores1 = self.Matching_Layer(mdesc0, mdesc1,match_gt)
                ...
                if len(a_ids_) > 0:
                    ...
                    mdesc0_ = mdesc0_.squeeze(2).squeeze(2)[None].transpose(1, 2)
                    scores_, indices0_, indices1_, mscores0_, mscores1_ = self.Matching_Layer(mdesc0_, mdesc1, match_gt_a,ignore=True)
                    match_loss_, hard_loss_ = self.Matching_Layer.loss
                    match_loss.append(match_loss_)
                ...
                if len(b_ids_) > 0:
                    ...
                    mdesc1_ = mdesc1_.squeeze(2).squeeze(2)[None].transpose(1, 2)
                    scores_, indices0_, indices1_, mscores0_, mscores1_ = self.Matching_Layer(mdesc0, mdesc1_, match_gt_b,ignore=True)
                    match_loss_, hard_loss_ = self.Matching_Layer.loss
                    match_loss.append(match_loss_)
            else:
            ...
            matched_results['matches0'].append(indices0)
            matched_results['matches1'].append(indices1)
    ...
    def val_forward(self,img, target):
        for i in range(len(target)):
            for key,data in target[i].items():
                if torch.is_tensor(data):
                    target[i][key] = data.to(self.device)
        ...
        gt_den = self.Gaussian(x.cuda()) # [2, 1, 1280, 720]
        assert pre_map.size() == gt_den.size()
        pre_map = pre_map / self.dataset_cfg.DEN_FACTOR # [2, 1, 1280, 720]
        ...
        if (np.array(count_in_pair) > 0).all():
            ...
            scores =None
        matched_results = {'matches0': indices0, 'matches1': indices1, 'matching_scores0': mscores0, 'matching_scores1': mscores1, 'gt_matched': match_gt['a2b'],
                           'gt_outflow': len(match_gt['un_a']), 'gt_inflow': len(match_gt['un_b']), 'pre_outflow':pre_outflow, 'pre_inflow': pre_inflow,
                           'pre_points': pre_points, 'pre_map': pre_map, 'target': target, 'scores':scores, 'match_gt': match_gt}
    ...
    def test_forward(self, img):
        ...
        if (np.array(count_in_pair) > 0).all():
            ...
            pre_outflow = count_in_pair[0].item()
            pre_inflow = count_in_pair[1].item()
        matched_results = {'matches0': indices0, 'matches1': indices1, 'matching_scores0': mscores0, 'matching_scores1': mscores1, 'pre_outflow': pre_outflow, 'pre_inflow': pre_inflow,
                           'pre_points': pre_points, 'pre_map': pre_map, 'scores': scores}
