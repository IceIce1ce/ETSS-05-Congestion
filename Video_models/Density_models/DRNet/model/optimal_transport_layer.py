...
class Optimal_Transport_Layer(nn.Module):
    ...
    def forward(self, mdesc0, mdesc1, match_gt=None, ignore=False): # [1, 256, 35], [1, 256, 34]
        sim_matrix = torch.einsum('bdn,bdm->bnm', mdesc0, mdesc1)
        scores = sim_matrix / self.feature_dim ** .5
        scores = log_optimal_transport(scores, self.bin_score, iters=self.iters)
        ...
        return scores, indices0.squeeze(0), indices1.squeeze(0), mscores0.squeeze(0), mscores1.squeeze(0) # [36, 35], [35], [34], [35], [34]

def log_sinkhorn_iterations(Z, log_mu, log_nu, iters: int): # [1, 36, 35], [1, 36], [1, 35], 100
    log_u, log_v = torch.zeros_like(log_mu), torch.zeros_like(log_nu)
    for _ in range(iters):
        log_u = log_mu - torch.logsumexp(Z + log_v.unsqueeze(1), dim=2)
        log_v = log_nu - torch.logsumexp(Z + log_u.unsqueeze(2), dim=1)
    return Z + log_u.unsqueeze(2) + log_v.unsqueeze(1) # [1, 36, 35]

def log_optimal_transport(scores, alpha, iters: int): # [1, 35, 34], [1], 100
    b, m, n = scores.shape
