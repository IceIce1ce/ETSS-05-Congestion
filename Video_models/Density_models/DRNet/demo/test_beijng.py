...
def test(cfg_data):
    ...
    sub_dataset = TestDataset(scene_name='demo', base_path=args.input_dir, img_transform=img_transform, interval=args.test_intervals, target=False, datasetname=args.type_dataset)
        ...
        for vi, data in enumerate(gen_tqdm, 0):
            ...
            with torch.no_grad():
                ...
                if kpts0.shape[0] > 0 and kpts1.shape[0] > 0:
                    save_visImg(kpts0, kpts1, matches, confidence, vi, img[0].clone(), img[1].clone(), args.test_intervals, args.output_dir, time, cnt, pred_dict, restore_transform)
...
def save_visImg(kpts0, kpts1, matches, confidence, vi, last_frame, cur_frame, intervals, save_path, time=None, cnt=None, pred_dict=None, restore_transform=None):
    valid = matches > -1
    ...
    outflow_cnt = 0
    inflow_cnt = 0
    total_cnt = pred_dict['first_frame']
    for idx, data in enumerate(pred_dict['outflow']):
        if idx % intervals == 0:
            outflow_cnt += data
    for idx, data in enumerate(pred_dict['inflow']):
        if idx % intervals == 0:
            total_cnt += data
            inflow_cnt += data
    text = ['pedestrian_cnt: {}'.format(np.around(total_cnt,2)), 'previous_frame: {}s'.format(vi / intervals, 2),
            'inflow_tmp: {}'.format(np.around(pred_dict['inflow'][-1], 2)), 'outflow_cnt: {}'.format(np.around(outflow_cnt,2)),
            'inflow_cnt: {}'.format(np.around(inflow_cnt,2)), 'outflow_tmp: {}'.format(np.around(pred_dict['outflow'][-1], 2)),
    ...
    plt.xlim(0, 200)
    plt.ylim(0, 1000)
    plt.text(3,830, r"Pedestrian number: $N= N_{0} + \sum N_{in}$", fontsize=32)
    ax.set_xlabel('Time (s)', fontsize=32)
    ax.set_ylabel('pedestrian number',fontsize=32)
    ax.plot(time, cnt, 'b', lw=4)
    plt.legend(['Predicted'], loc = 'upper left', fontsize=28)
    plt.savefig(f'{args.output_dir}/{vi}.jpg', dpi=100)
    curve = fig2data(fig)
    ...
    for i, t in enumerate(text):
        if i == 0:
            cv2.putText(out, t, (600, 70), cv2.FONT_HERSHEY_DUPLEX, 2, txt_color_fg, 2, cv2.LINE_AA)
...
if __name__=='__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='saved_demo')
    parser.add_argument('--test_intervals', type=int, default=90)
