...
class backbone_FPN(nn.Module):
    def __init__(self,cfg):
        ...
        for i in range(len(in_channels)):
            self.scale_loc_bottleneck.append(nn.Sequential(nn.Dropout2d(0.2), ResBlock(in_dim=192, out_dim=128, dilation=0, norm="bn"),
                                                           ResBlock(in_dim=128, out_dim=128, dilation=0, norm="bn")))
            self.scale_loc_head.append(nn.Sequential(nn.ConvTranspose2d(128, 64, 2, stride=2, padding=0, output_padding=0, bias=False),
    ...
    def forward(self, x): # [4, 3, 768, 1024]
        ...
        return f_mask, den_scale, f_den # [4, 128, 192, 256], [4, 1, 768, 1024], [4, 128, 192, 256]