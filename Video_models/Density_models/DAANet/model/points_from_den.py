...
class get_ROI_and_MatchInfo(object):
    ...
    def __call__(self, target_a, target_b, noise=None, shape=None):
        gt_a, gt_b = target_a['points'], target_b['points'] # [14, 2], [9, 2]
        ...
        matched_a2b = torch.stack([matched_a, matched_b],1) # [8, 2]
        unmatched0 = torch.where(dis.min(1)[0] > 0)[0] # [6]
        unmatched1 = torch.where(dis.min(0)[0] > 0)[0] # [1]
