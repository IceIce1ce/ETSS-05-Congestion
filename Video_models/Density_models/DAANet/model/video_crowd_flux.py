...
class DAANet(nn.Module):
    def __init__(self, cfg):
        ...
        num_feat = 128
        self.deformable_alignment = MOFAlignment(cfg, num_feat)
        self.mask_predict_layer = nn.Sequential(nn.Dropout2d(0.2), ResBlock(in_dim=128, out_dim=64, dilation=0, norm="bn"), ResBlock(in_dim=64, out_dim=32, dilation=0, norm="bn"),
                                                ...
                                                nn.BatchNorm2d(4, momentum=BN_MOMENTUM), nn.Conv2d(4, 1, kernel_size=1, stride=1, padding=0))
        self.ASAM = nn.Sequential(nn.Conv2d(384, 64, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(64, momentum=BN_MOMENTUM), nn.ReLU(inplace=True),
                                  nn.Conv2d(64, 32, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(32, momentum=BN_MOMENTUM), nn.ReLU(inplace=True),
                                  nn.Conv2d(32, 3, kernel_size=1, stride=1, padding=0))
        nn.init.constant_(self.ASAM[6].weight, 0.)
        nn.init.constant_(self.ASAM[6].bias, 0.)

    def DDA(self, feature, attns): # [4, 128, 192, 256], [4, 3, 768, 1024]
        feature1 = []
        feature2 = []
        ...
        return feature1, feature2 # [2, 128, 192, 256], [2, 128, 192, 256]

    def forward(self, img): # [4, 3, 768, 1024]
        ...
        f_attn = torch.cat([feature_den[0],  feature_den[1], feature_den[2]], dim=1)
...
class MOFAlignment(nn.Module):
    def __init__(self, cfg, num_feat):
        ...
        for scale in range(3):
            self.feature_head.append(nn.Sequential(nn.Dropout2d(0.2), ResBlock(in_dim=self.channel_size, out_dim=self.channel_size, dilation=0, norm="bn"),
                                                   nn.Conv2d(self.channel_size, self.channel_size, kernel_size=3, stride=1, padding=1, bias=False),
                                                   nn.BatchNorm2d(self.channel_size, momentum=BN_MOMENTUM), nn.ReLU(inplace=True),
                                                   nn.Conv2d(self.channel_size, self.channel_size, kernel_size=3, stride=1, padding=1)))
            self.multi_scale_dcn_alignment.append(MultiScaleDeformableConv(cfg, self.channel_size, self.channel_size, offset_groups=4, kernel_size=3, mult_column_offset=True, scale=scale))
        self.weight_conv = nn.Sequential(ResBlock(in_dim=self.channel_size*6, out_dim=self.channel_size*3, dilation=0, norm="bn"),
    ...
    def forward(self, f1, f2): # [2, 128, 192, 256], [2, 128, 192, 256]
        f_flow = []
        b_flow = []
        f1_aligned = []
        f2_aligned = []
        ...
        return f_mask, f_flow, b_flow, f1, f2 # [4, 128, 192, 256], [2, 72, 768, 1024], [2, 72, 768, 1024], [2, 384, 192, 256], [2, 384, 192, 256]