...
def plot_boxes(cur_frame, head_map, points, ids, body_map={}, text=True):
    ...
    for index,  t_dim in enumerate(head_map):
        ...
        cv2.rectangle(plotting_im, (startX, startY), (endX, endY), (0, 255, 0), 2)
        cur_centroid = tuple([(startX + endX) // 2, (startY + endY) // 2])
        if text:
            cv2.putText(plotting_im, str(ids[index]), cur_centroid, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    for index,  t_dim in enumerate(points):
        X, Y, = [int(i) for i in t_dim]
        cv2.circle(plotting_im, (X, Y), 2, (0, 0, 255), 2)
...
def CroHead(root):
    ...
    for sub_scene in sub_scenes:
        print('Visualize scene:', sub_scene)
        scene_dir = os.path.join('vis_crohd', root.split('/')[-1], sub_scene)
        ...
        for img_id in imgs_id:
            ...
            for idx, label in enumerate(labels_point):
                ...
                id_list.append(int(label[1]))
            img = cv2.imread(img_path)
            img = plot_boxes(img, annotations, points, id_list)
            save_path = os.path.join(scene_dir, img_id)
...
def SensorCrowd(video_path, label_path):
    Info_dict = {}
    time = 0
    for scene in sorted(os.listdir(video_path)[51:]):
        print('Visualize scene:', scene)
        scene_dir = os.path.join('vis_sense', scene)
        if not os.path.exists(scene_dir):
            os.makedirs(scene_dir)
        gts = defaultdict(list)
        ...
        for img_id in img_ids:
            ...
            ids = box_and_point[:, 6].astype(np.int32)
            id_list.append(ids)
            img = cv2.imread(img_path)
...
if __name__ =='__main__':
    # SensorCrowd(video_path='data/Sense/video_ori', label_path='data/Sense/label_list_all')
    # Infor_statistics()
    # SENSE_train_val_test()
    CroHead(root='data/HT21/train')