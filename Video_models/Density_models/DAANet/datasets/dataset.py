...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img, target # [3, 768, 1024], [1]

class TestDataset(data.Dataset):
    def __init__(self, scene_name, base_path, img_transform=None, interval=1, target=True, datasetname='Empty'):
...
def SENSE_ImgPath_and_Target(base_path, i):
    ...
    img_ids.sort()
    gts = defaultdict(list)
    with open(root.replace('video_ori', 'label_list_all') + '.txt', 'r') as f:
        lines = f.readlines()
        for lin in lines:
    ...
    for img_id in img_ids:
        ...
        if ids.size(0) > 0:
            sigma = 0.6 * torch.stack([(box_and_point[:, 2] - box_and_point[:, 0]) / 2, (box_and_point[:, 3] - box_and_point[:, 1]) / 2], 1).min(1)[0]
        else:
            sigma = torch.tensor([])
        img_path.append(single_path)
        labels.append({'scene_name': i,'frame': int(img_id.split('.')[0].replace('_resize','')), 'person_id': ids, 'points': points, 'sigma': sigma})
